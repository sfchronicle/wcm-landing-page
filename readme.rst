...
.. code:: sh
    ...
    git clone git@github.com:sfchronicle/wcm-landing-page

If it works, you should be able to ``ls ~/.grunt-init/wcm-landing-page`` and get back a list of files. That's it! Now let's start a sample project to see how it all
works.
...
-  ``docs`` - Download data from Google Doc and save as JSON files
...
The WCM task ``grunt wcm`` builds the code needed for the Freeform into a file called ``wcm.html`` in the build folder. The task removes the code that already exists within WCM and minifies it.
